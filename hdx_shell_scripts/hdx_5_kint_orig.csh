...
# orig_res.list - listing resnames & resids; HSE/HSD ---> HIS, leave HSP as they are.  #
...
while ( $i < $fres )
    ...
    if ( $test == 0 ) then
        ...
        set resni = `awk '{if($1=='$i') print $2}' ../orig_res.list` # resname of res$i
        set resnj = `awk '{if($1=='$j') print $2}' ../orig_res.list` # resname of res$j
        set resnk = `awk '{if($1=='$k') print $2}' ../orig_res.list` # resname of res$k
        ...
        if ( $resnk == "PRO" ) then # res$k is PRO; res$i is to the left of PRO
            ...
            set lgAL = 0.00
            set lgBL = `grep HIS ../hdx_sd-exrate.dat | awk '{print $4}'`
...
#            set lgBL = `grep HIS ../hdx_sd-exrate_new.dat | awk '{print $4}'`
        else # res$k is not PRO/HSE/HSD
            set lgAL = `awk '{if($1=="'$resnk'") print $2}' ../hdx_sd-exrate.dat`
            set lgBL = `awk '{if($1=="'$resnk'") print $4}' ../hdx_sd-exrate.dat`
        endif
        # determine lgAR, lgBR
        if ( $resnj == "PRO" ) then # res$j is PRO; res$i is to the right of PRO
            set lgAR = `grep PRO ../hdx_sd-exrate.dat | sed '2d' | awk '{print $3}'` # trans-PRO
            set lgBR = `grep PRO ../hdx_sd-exrate.dat | sed '2d' | awk '{print $5}'` # trans-PRO
            ...
            set lgBR = `grep HIS ../hdx_sd-exrate.dat | awk '{print $5}'`
        else # res$j is not PRO/HSE/HSD
            set lgAR = `awk '{if($1=="'$resnj'") print $3}' ../hdx_sd-exrate.dat`
            set lgBR = `awk '{if($1=="'$resnj'") print $5}' ../hdx_sd-exrate.dat`
        endif
