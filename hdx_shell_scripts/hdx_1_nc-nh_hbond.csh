...
set dire = 1-nc-nhbonds                 # where you put output
...
set cuth = 0.35                         # Cutoff for Nhbonds, heavy atom - heavy atom dist
set angh = 30                           # Cutoff angle for Nh, H - D - A, deg
...
# Copy Nc from previous folder!
rm -f $dire/nc_res*.dat
cp 1-nc-nh/nc_res*.dat $dire
...
while ( $i <= $fres )
    ...
    if ( $test == 0 ) then
        ...
        # Make *.ndx for res$i
        echo "t H & a HN & r $i" > tmp.txt        # define amide H of res$i
        echo "t NH1 & a N & r $i" >> tmp.txt      # define amide nitrogen of res$i
        echo "10 | 11" >> tmp.txt
        echo "name 12 amide_nh_res$i" >> tmp.txt
        echo "t N* O* & ! 11" >> tmp.txt          # define O/N of all residues except amide N of res$i
        echo "del 10-11" >> tmp.txt
        echo "name 11 nitrogen_oxygen" >> tmp.txt
        echo "q" >> tmp.txt
        gmx make_ndx -f $tpr -o res$i.ndx < tmp.txt  # make *.ndx for residue $i. MUST use *.tpr here
        echo "10" > tmp.txt
        echo "11" >> tmp.txt
        # Calcualte Nhbonds for res$i
        gmx hbond -f $trj -s $tpr -n res$i.ndx -a $angh -r $cuth -num tmp.xvg -xvg none < tmp.txt
        @ j = $i - 1 # get real resid
        # NR/100 = time in ns for 10 ps intervals
        awk '{print NR/100, $2}' tmp.xvg > $dire/nh_res$j.dat
        rm -f tmp.* res$i.ndx
    endif
    nextnh:
    @ i++
