...
# Set variables
set dire = "7-d-fraction-seg-orig" # where you put output
...
while ( $seg <= $nseg )
    ...
    @ sres = $sres - 1 # Segres index shifts from MD
    @ fres = $fres - 1 # Segres index shifts from MD
    @ i = $sres  # Segres index is same as MD
    while ( $i <= $fres )
        ...
        if ( $test == 0 ) then # in case that res$i is PRO, got to next residue
            ...
            if ( $i == $sres ) then # This misses the case where a starting residue is a PRO
                awk '{for(i=1;i<=NF;i++) a[i,NR]=$i}END{for(i=1;i<=NF;i++) {for(j=1;j<=NR;j++) printf a[i,j] " ";print ""}}' ../6-d-fraction-orig/df_res$i.dat > tmp
            else
                awk '{print $2}' ../6-d-fraction-orig/df_res$i.dat | awk '{for(i=1;i<=NF;i++) a[i,NR]=$i}END{for(i=1;i<=NF;i++) {for(j=1;j<=NR;j++) printf a[i,j] " ";print ""}}' >> tmp
            endif
        endif
        nextres:
        @ i++
    end
    @ sres = $sres + 1 # Segres index returns as MD
    @ fres = $fres + 1 # Segres index returns as MD
